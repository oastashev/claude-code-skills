...
2. **После DDD — стоп.** Не запускай `/audit`, не создавай `openspec/`, не пиши CLAUDE.md. Заверши итогом и фразой «Когда будешь готов — запусти /audit».
...
- Прочитай `docs/STATUS.md` и `docs/00-exploration.md` целиком — exploration единственный документ, который читается полностью (см. «Что читать под каждый документ»). Если exploration отсутствует — остановись и предложи запустить `/explore`. Если пользователь настаивает работать по описанию из чата — можно, но зафиксируй в BRD, что шаг exploration пропущен, и ожидай больше уточняющих вопросов.
...
| DDD | SDD целиком; SAD §4, §7; TRD §4 (NFR) |
...
Здесь рождаются формулировки, которые `/kickoff` перенесёт в `openspec/specs/<capability>/spec.md` почти дословно. Поэтому каждое функциональное требование записывай по одному из шаблонов EARS (Easy Approach to Requirements Syntax), по-английски, с одним `SHALL` и одним действием:
...
Правила: одно требование — одно поведение (никаких «and/or» с двумя действиями); измеримо и проверяемо (не «быстро», а «within 2 seconds at p95»); без слов `should`, `may`, `user-friendly`, `etc.`, `appropriate`; `<system>` — конкретное имя системы или компонента. Каждое FR получает 1–3 сценария приёмки в формате `WHEN … THEN …` — они станут `#### Scenario:` в OpenSpec.
...
## 4. Нефункциональные требования   NFR-xxx, тоже в EARS, по категориям: производительность, надёжность, безопасность, масштабируемость, доступность, локализация, наблюдаемость, сопровождаемость
...
## 4. Выбранный стек и обоснование   язык, фреймворки, БД, хостинг, CI — с альтернативами, которые отклонены
## 5. Архитектурные решения (ADR)   ADR-xx: контекст → решение → последствия
## 6. Сквозные аспекты   аутентификация, авторизация, логирование, обработка ошибок, конфигурация, секреты
## 7. Развёртывание и окружения   dev / staging / prod, схема
...
```markdown
## 1. Структура репозитория   дерево каталогов с назначением
## 2. Детальный дизайн модулей   **только для модулей первых двух changes (CH-01, CH-02)**: файлы, публичный интерфейс (сигнатуры), внутренние типы, алгоритмы нетривиальных частей, ошибки. Для остальных changes — строка в §7: дизайн достроит Claude Code перед началом change по реально написанному коду, а упреждающий дизайн к тому моменту всё равно устареет
## 3. Схема БД / хранилища   DDL или схема миграций, индексы, сиды
## 4. Спецификации API   единственное место с полными контрактами: OpenAPI-фрагменты или таблицы, примеры запросов/ответов, ошибки, авторизация — для интерфейсов CH-01 и CH-02; остальные описываются, когда до них дойдёт очередь
...
## 7. План реализации   упорядоченный список изменений (changes) для OpenSpec:
   ...
   | CH-01 | project-foundation | каркас, CI, БД | platform | NFR-001…NFR-004 (сквозные) | — | S |
   | CH-02 | user-registration | … | CAP-01 | FR-001…FR-006, NFR-007 | CH-01 | M |
   Первый change — всегда каркас проекта; к нему относятся сквозные NFR (CI, тесты, наблюдаемость, безопасность платформы) под служебной capability `platform`. Каждый следующий — законченный вертикальный срез, который можно проверить. Каждое Must/Should FR/NFR попадает ровно в один change.
## 8. Задачи по изменениям   T-xxx с описанием, файлами, критерием готовности, ссылкой на FR — **только для CH-01 и CH-02**. Задачи дальних changes устареют ровно так же, как их дизайн, и `/kickoff` для них `tasks.md` намеренно не генерирует
## 9. Definition of Done   (тесты, линтер, документация, spec обновлён)
## 10. Соглашения кодовой базы   стиль, именование, коммиты, ветки — то, что попадёт в CLAUDE.md
```
...
4. Утверждено → `approved` в STATUS, переходи к следующему. Правки → исправь, версия +1, снова п. 2–3. Остановка → STATUS остаётся `draft`, заверши ответ.
...
- Не писать требования прозой или по-русски внутри EARS-таблицы: `/kickoff` копирует их в спецификации дословно, валидатор OpenSpec в строгом режиме требует `SHALL`/`MUST` в тексте требования, а Claude Code будет писать новые требования по образцу существующих.
...
- Не расписывать детальный дизайн и задачи для всех changes сразу — только для первых двух.
